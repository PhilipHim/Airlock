...
from agent.orgs import aggregate_count, has_batch, private_view, shareable_return
...
def _name_motion() -> dict[str, Any]:
    # Intentionally leaky payload. Name in claim must not be stored.
    return {
        "id": "m1",
        "from_org": "org_a",
        "customer_name": "Anna Müller",
        "claim": "Anna Müller returned the earbuds. Battery swollen.",
    }


def _return_motion() -> dict[str, Any]:
...
def move(state: dict[str, Any]) -> dict[str, Any]:
    ...
    proposals.append(_return_motion())
...
def second(state: dict[str, Any]) -> dict[str, Any]:
    """Org B seconds only a batch it has in its own factory file."""
    private_view("org_b")
    for motion in state["motions"]:
        if motion.get("status") != "moved":
            continue
        batch_id = motion.get("batch_id")
        if has_batch("org_b", batch_id if isinstance(batch_id, str) else None) and inspect(
            motion
        ).allowed:
...
def chair(state: dict[str, Any], motion_id: str = "m2") -> dict[str, Any]:
    ...
    for motion in state["motions"]:
        ...
        record = {
            ...
            "claim": motion["claim"],
            "batch_id": motion.get("batch_id"),
            "product_type": motion.get("product_type"),
        }
...
def snapshot(state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "org_b": {
            ...
            "note": "Only Agent B sees its factory lots.",
        },
        "motions": state["motions"],
        "standing_orders": state["standing_orders"],
        "public_record": state["public_record"],
